...
export default function ProjectsPage() {
  ...
  const handlePasswordSetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh projects after password setup
      fetchProjects();
    } catch (error) {
  ...
  // Add this useEffect to check if password setup is needed
  useEffect(() => {
    const checkPasswordSetup = async () => {
      if (session?.user?.id) {
        const response = await fetch('/api/user/profile');
        const data = await response.json();
        if (data.authProvider === 'google' && !data.password) {
  ...
  return (
    <div className="min-h-screen bg-[#292929] p-6">
      ...
      <CustomDialog open={isPromptModalOpen} onOpenChange={setIsPromptModalOpen}>
        <div className="space-y-4">
      ...
      <CustomDialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <div className="space-y-4">
          <h2 className="text-center text-white text-2xl font-semibold">
            To a new beginning
          </h2>
      ...
      <CustomDialog open={showPasswordSetupModal} onOpenChange={setShowPasswordSetupModal}>
        <div className="space-y-4">
          <div className="text-center">
            <h2 className="text-2xl font-semibold text-white">Set Up Password</h2>
